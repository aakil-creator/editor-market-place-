#!/usr/bin/env python3
"""Test if root route works on port 8000 app."""
import sys, os
...
async def root():
    ...
    print(f'root called: f={f}, exists={f.exists()}', flush=True)
    if not f.exists():
...
async def spa_fallback(full_path: str):
    if full_path == '':
        f = STATIC_DIR / 'index.html'
        print(f'spa_fallback empty: f={f}, exists={f.exists()}', flush=True)
        return FileResponse(f)
...
print('Starting on 127.0.0.1:9988...', flush=True)
uvicorn.run(app, host='127.0.0.1', port=9988, log_level='debug')
